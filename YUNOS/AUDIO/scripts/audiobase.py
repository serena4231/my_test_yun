...
Audio_Comm={
    'OPENAUDIO':'adb -host shell sendlink "page://audioplayer.yunos.com/audioplayer"',
    'PUSHAUDIO':'adb -host push {0} {1}'.format(path_local,path_devcices),
    'PLAY_ALL':'adb -host shell input tap 100 100',
    'PLAY_BUTTON':'adb -host shell input tap 660 680',
    'PROPERTY_MISC':'adb -host shell input tap 1200 150',
    'SEEK_MID':'adb -host shell input tap 662 650',
    'SEEK_START':'adb -host shell input tap 5 650',
    'SEEK_END':'adb -host shell input tap 1100 650',
    'DELETE_CHOICE':'adb -host shell input tap 1200 108',
    'DELETE_CLICK':'adb -host shell input tap 1240 30',
    'DELETE_AUDIO':'adb -host shell input tap 770 700',
    'CONFIRM_DELETE':'adb -host shell input tap 700 440',
    'VOLUME_SETTING':'adb -host shell input tap 100 440',
}
...
class Audiobase():
    ...
    def seek__player_start(self):
        ...
        status = checkfile.check_stauts('text="00:03/01:00"')
        if status == True:
            logger.info('Seek the audio to start is success')
        else:
            logger.error('Seek the audio to start is failed')
            exit(1)
    ...
    def seek_player_end(self):
        ...
        time.sleep(3)
        status = checkfile.check_stauts('text="00:55/01:00"')
        if status == True:
            logger.info('Seek the audio to near the end is success')
        else:
            logger.error('Seek the audio to near the end is failed')
            exit(1)
...
Recorder_Comm={'OPEN_RECORDER':'adb -host shell sendlink page://recorder.yunos.com/recorder',
               ...
               'SAVE_RECORDER':'adb -host shell input tap 700 430',
...
class Recorbase():
    ...
    def delete_recorder(self):
        ...
        status,output,error=Command(Recorder_Comm['CONFRIM_DELETE']).start()
        BaseAPI().check_adb_command('ret = true',output)
